...
1. `intro/` — вводная часть курса (особая публичная страница, всегда доступна полностью без paywall).
...
- Скорость чтения задаётся в конфиге (`config/site.json`), по умолчанию 150 слов/мин (вдумчивое чтение учебного материала).
...
### 7.2. Навигация «Далее» через Progress Widget

- Формируется линейный список всех пунктов курса (intro → course → appendix).
- Навигация в premium-версии **однонаправленная** — пользователь движется только вперед.
- **Кнопка «Назад» полностью удалена** из интерфейса.
- Возврат на предыдущие страницы возможен только через:
  - боковое меню курса;
  - браузерную кнопку «Назад».

Для каждой страницы в HTML добавляются data-атрибуты:
- `data-page-type` — тип страницы (`free`, `premium`, `recommendation`, `intro-free`, `intro-premium`);
- `data-button-text` — текст кнопки Progress Widget;
- `data-next-page` — URL следующей страницы.

Progress Widget показывает разный текст CTA в зависимости от типа страницы:
- **Free версия**: «Войти в полную версию» → открывает модальное окно оплаты;
- **Premium версия**: «Далее» → переход на следующую страницу;
- **Рекомендации**: «Открыть курс» → переход на intro или последнюю сохраненную позицию.

Для рекомендаций используется «умная» навигация:
- При прочтении premium-страниц позиция сохраняется в `localStorage`;
- При клике «Открыть курс» пользователь попадает на последнюю прочитанную страницу (если есть) или на intro.

В free-версии кнопки «Назад/Далее» не выводятся — навигация происходит только через Progress Widget.
...
1. Build-скрипт `build-reco`:
   ...
   - формирует `dist/shared/recommendations.json` со списком карточек.

2. **Динамическая загрузка на фронте:**

   - HTML-шаблон содержит только пустой контейнер `.stack-carousel`;
   - при загрузке страницы JavaScript:
     1. запрашивает `/shared/recommendations.json`;
     2. автоматически создаёт слайды (по 2 карточки на слайд);
     3. генерирует точки индикатора;
     4. подключает интерактивность (автопрокрутка, свайпы, клики).
   ...
   - каждая карточка — ссылка на `/recommendations/<slug>.html`;
   - количество слайдов = `Math.ceil(рекомендаций / 2)`;
   - при ошибке загрузки JSON используется fallback на статичный контент.
...
### 10.2. Progress Widget — навигация «Далее»

> **Примечание:** Кнопка «Назад» полностью удалена из интерфейса. Навигация стала однонаправленной для упрощения UX и акцента на линейном прохождении материала.

1. **Расположение:**
   - Progress Widget отображается как плавающий элемент справа от текста;
   - показывает процент прочтения (0–100%);
   - при достижении 100% морфится в кнопку с текстом действия.

2. **Текст кнопки зависит от типа страницы:**

   | Тип страницы | data-page-type | Текст кнопки | Действие |
   |--------------|----------------|--------------|----------|
   | Free версия курса | `free` | «Войти в полную версию» | Открывает модальное окно оплаты |
   | Free intro | `intro-free` | «Войти в полную версию» | Открывает модальное окно оплаты |
   | Premium версия | `premium` | «Далее» | Переход на следующую страницу |
   | Premium intro | `intro-premium` | «Далее» | Переход на первую страницу курса |
   | Рекомендации | `recommendation` | «Открыть курс» | Переход на intro или сохраненную позицию |

3. **Умная навигация для рекомендаций:**
   - при прочтении premium-страниц последняя позиция сохраняется в `localStorage` (ключ `toosmart_last_premium_position`);
   - при клике «Открыть курс» проверяется наличие сохраненной позиции;
   - если позиция есть — переход туда, иначе — на intro.

4. **Возврат на предыдущие страницы:**
   - через боковое меню курса (всегда доступно);
   - через браузерную кнопку «Назад».

5. **Отслеживание:**
   - Progress Widget использует `data-analytics` атрибуты на карточках рекомендаций;
   - возможно отслеживание переходов через стандартные механизмы аналитики.

6. **Состояния:**
   - до 100%: клик докручивает страницу до конца;
   - при 100%: клик выполняет действие согласно типу страницы;
   - на последней странице курса `data-next-page` может быть пустым — в этом случае кнопка не активируется.
