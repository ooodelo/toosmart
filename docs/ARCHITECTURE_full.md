...
Отдельно зафиксировать: все текстовые материалы хранятся в виде Markdown-файлов внутри `content/` и используются **только на этапе сборки**. На прод-сервер деплоятся уже готовые статические HTML/JS/JSON-файлы, никакого динамического парсинга Markdown в рантайме не предполагается.
...
В `package.json` должны быть как минимум следующие команды сборки:

- `npm run build:all` — полная пересборка free + premium, генерация всех HTML и JSON (включая `recommendations.json`).
- `npm run build:free` — пересборка только бесплатной версии.
- `npm run build:premium` — пересборка только платной версии.
- `npm run build:reco` — пересборка только рекомендаций и JSON для карусели, без трогания остального курса.

Конкретные имена команд могут отличаться, но функциональность должна сохраняться: возможность быстро пересобирать только рекомендации и полную пересборку обоих сайтов.
...
# Дополнения архитектуры — версия 2.0 (preview и время чтения)

Этот раздел дополняет существующий ARCHITECTURE.md и уточняет поведение бесплатной версии для платных разделов курса.

## Обновлённая логика превью платных разделов

Для платных разделов курса в бесплатной версии страницы должны строиться так:

1. Из Markdown-файла отделяется логическое введение:
   - H2 «Введение» + абзацы до следующего H2 или `---`, если такой заголовок есть;
   - первые 2–3 абзаца после H1 (и возможного разделителя `---`) до первого H2, если H2 «Введение» отсутствует;
   - вариант с H1 → `---` → H2 обрабатывается как «первый H2 + 1–2 абзаца под ним».

   Этот блок рендерится в бесплатной версии целиком и является основным SEO-контентом страницы.

2. После введения формируется blur-фрагмент:
   - из оставшегося текста берутся следующие абзацы до достижения примерно 300 px высоты на десктопе (порог по словам/абзацам задаётся в конфиге сборки);
   - фрагмент рендерится в контейнер `.blurred-content` (или эквивалент) с градиентом и CSS blur;
   - внутри контейнера текст оборачивается в `<!--noindex-->…<!--/noindex-->`, а самому контейнеру добавляется `data-nosnippet`.

   Полный текст раздела за пределами этого blur-фрагмента в бесплатной сборке **не присутствует**.

3. Под blur-фрагментом располагается CTA-кнопка «Получить полный доступ», которая открывает модальное окно оплаты (описано в основном ТЗ).

В платной версии (`dist/premium`) раздел рендерится целиком без blur и ограничений.

## Время чтения

Модуль, считающий время чтения раздела (по количеству слов и заданной скорости чтения), становится общим для:

- меню структуры курса (время показывается рядом с каждым разделом в free и premium);
- paywall-блока бесплатной версии (над blur-фрагментом отображается строка «Осталось N минут чтения» с тем же значением N).

Все остальные разделы ARCHITECTURE.md, не связанные с логикой preview и временем чтения, остаются без изменений и считаются актуальными.
