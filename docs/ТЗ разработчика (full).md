...
**ПЛАТНАЯ версия** (template-full.html, НЕТ payment-modal):

1. При загрузке страницы отображается **кружок с процентами** (0%);
2. При скролле процент увеличивается от 0% до 100%;
3. При достижении 100% кружок **превращается в кнопку "Далее"** через плавную анимацию;
4. При клике выполняется переход на следующую страницу курса.

**БЕСПЛАТНАЯ версия** (template-paywall.html, ЕСТЬ payment-modal):

1. При загрузке страницы сразу отображается **кнопка "Получить полный доступ"** (без кружка и прогресса);
2. Кнопка центрируется по горизонтали во всех режимах;
3. При клике открывается модальное окно оплаты;
4. Скролл НЕ отслеживается, прогресс НЕ показывается.

**Рекомендации** (в платной версии, data-recommendations="true"):

1. Показывает кружок с прогрессом 0% → 100% (как в платной версии);
2. При клике на кнопку "Далее" может открывать модальное окно покупки (опционально);
3. Используется атрибут `data-recommendations="true"` на элементе `.text-box` или `body`.
...
- **Кнопка "Далее" при 100%:** также остается по левому краю (НЕ центрируется)
  - Используется та же CSS переменная `--pw-float-left`
  - Кружок и кнопка всегда выравнены по одному левому краю

**Mobile (< 768px) — БЕСПЛАТНАЯ версия:**
- Кнопка "Получить полный доступ" всегда центрируется по горизонтали (left: 50%, transform: translate(-50%, 0))
- Позиционирование НЕ зависит от dock-handle

#### Текст кнопки в разных версиях

Текст кнопки зависит от версии сайта и типа контента:

* **БЕСПЛАТНАЯ версия (paywall):** «Получить полный доступ» — при клике открывает модальное окно оплаты;
* **ПЛАТНАЯ версия, course разделы:** «Далее» — при клике переходит на следующий раздел курса;
* **ПЛАТНАЯ версия, intro:** «Далее» — переходит на первый раздел курса;
* **ПЛАТНАЯ версия, recommendations:** «Вернуться к курсу» — возврат на главную или предыдущий раздел.
