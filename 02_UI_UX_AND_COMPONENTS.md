...
- `src/template-paywall.html` — шаблон для premium/закрытых страниц. Структура аналогична, но включает CTA и области для навигации «Назад/Далее» в соответствии с цепочкой курса.
...
## JS-модули
- `src/script.js` — основной скрипт, который исполняется на собранных страницах. Он вычисляет `data-mode` (mobile/tablet/desktop/desktop-wide) и `data-input` (touch/pointer), управляет классом `menu-open`, контролирует flyout-навигацию по секциям и навигационные ссылки «Назад/Далее». Внутри определён контроллер состояния меню (aria-атрибуты, синхронизация DOM), трекинг активной секции и обработчики свайпов/клавиатуры.
...
- Для premium билд подставляет ссылки «Назад/Далее» согласно цепочке `intro → course → appendix`. `script.js` читает `data-next-page` в `article` для кнопки CTA «Далее» на free-страницах.
